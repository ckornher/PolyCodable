...
open class StandardPolymorphicCodingScheme<CodedType> : PolymorphicCodingScheme where CodedType: PolyCodable{
    ...
    /// Decode a dictionary with polymorphic values from a standard KeyedDecodingContainer.
    /// This method can be re-implemented for custom poymorphic coding schemes.  The key enum in this container must contain the descrimintor key.
    open func polymorphicDecode<Key, DictKey, PC>(from container: KeyedDecodingContainer<Key>,
                                         forKey key: Key) throws -> [DictKey: PC]
        where Key : CodingKey, DictKey: Codable & Hashable, PC : PolyCodable {

        let descrimintorArray = try container.decode( [DictKey: PolyCodableDescriminatorContainer<CodedType>].self, forKey: key )
        var desciminatorIterator = descrimintorArray.makeIterator()

        var decoded = [DictKey: PC]()
        var dictionaryContainer = try container.nestedUnkeyedContainer(forKey: key)

        while !dictionaryContainer.isAtEnd {
            guard let (dictKey1, descriminatorContainerValue) = desciminatorIterator.next() else {
                preconditionFailure("internal logic failure")
            }

            let entry: PolymorphicDictionaryEntry<DictKey,PC> = try descriminatorContainerValue.typeDescriminator.decodeNextDictionaryElement(from: &dictionaryContainer)
            assert(dictKey1 == entry.key)
